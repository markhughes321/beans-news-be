...
async function scrape() {
  const feedUrl = 'https://www.gcrmag.com/feed/';
  const sourceName = 'globalCoffeeReport';
  ...
  try {
    ...
    for (const item of feed.items) {
      try {
        ...
        if (item['content:encoded']) {
          ...
          const firstImg = $('figure img').first();
          if (firstImg.length) {
    ...
    logger.info(`Scraped ${articles.length} articles from Global Coffee Report`, { feedUrl });
    return articles;
  } catch (err) {
    logger.error('Error scraping Global Coffee Report RSS feed', { feedUrl, error: err.message });
    return [];
  }
}
